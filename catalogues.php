    ...
    <!-- NavBar -->
        ...
        <!--Catalogues -->
            <div class="container-fluid">
                ...
                <div class="col-md-12">
                    <h4 class="titleIndex">Catalogues</h4>
                </div>
                    <?php 
                        include("connexion.php");
                        if(isset($_GET["market"]))
                        {
                            $idMarkettt = $_GET["market"];
                             $sql ="SELECT m.image as logo, c.lib_catalogue as lib,c.id_catalogue as id , c.id_market FROM catalogue AS c INNER JOIN market AS m ON c.id_market = m.id_market WHERE c.id_market='$idMarkettt' ORDER BY c.id_market;";
                        }
                        else
                        {
                             $sql ="SELECT m.image as logo, c.lib_catalogue as lib,c.id_catalogue as id , c.id_market FROM catalogue AS c INNER JOIN market AS m ON c.id_market = m.id_market ORDER BY c.id_market;";
                        }
                       
                        $result = mysqli_query($link, $sql);
                        if(mysqli_num_rows($result) == 0)
                        {
                            echo "<h3 class='error' align ='center' style='margin : 100px 0 300px 38%;'>This market has no catalogues</h3>";
                        }
                        while($data = mysqli_fetch_assoc($result))
                        {
                            ...
                            $lib = $data["lib"];
                            $idCatalogue = $data["id"];
                            echo "<div class='col-md-2 offset-md-1' style='position : relative; right : 50px;margin-bottom : 20px;' id='catalogueBg'>";
                            echo "<h3 class='catalogueLib2'>".$lib."</h3>";
                            echo "<img id ='marketLogo2' src=\"MarketsLogos/".$image."\"><br><br>";
                            echo '<a href="catalogue.php?id='.$idCatalogue.'"><button type="button" class="btn btn-sm btn-warning" style="padding: 5px 10px;">View products<span></span></button></a><br><br>';
                            echo "</div>";
