...
<?php 
            ...
            {
                header("location: changePAdmin.php");
                exit();
            }
            if(isset($_SESSION["gestionnaire"]))
            {
                header("location: changePGestionnaire.php");
                exit();
            }
?>
<html lang="en">
...
<body style="background-image : url('slider/slide2.jpg');background-repeat : no-repeat;  background-size: 100% 100%;background-position: right 30% top -10% ;">    
    <!-- NavBar -->
        ...
        <!-- Join Form -->
    <h2 style ="background-color:rgba(0,0,0,0.7); text-decoration : none; color:white; margin : 12px 33%;padding: 10px 10px; border-radius: 3px; border : solid 2px black;"align="center">Change Password</h2>
        <form id="joinForm" method="post" action="changePassTreat.php" style ="color: white; background-color :rgba(0,0,0,0.6);">
        <?php 
            //forms
            echo '<br><br>
  <div class="form-group" align="center">
    <label for="firstname">Actual Password :</label><br>
    <input type="password" class="form-control-md" name="actual" size ="50" required>';
      if(isset($_COOKIE["errorTrue"]))
            {
                if($_COOKIE["errorTrue"] == 1)
                {
                    echo '<br><small class="error1" class="form-text text-muted">Wrong Password</small>';
                }
            }
            echo '
  </div><br><br>
    ...
    <input type="password" class="form-control-md" name="new" size ="50" required><br>
  </div><br><br>
    
    
    <div class="form-group" align="center">
    <label for="phone">Confirm New Password :</label><br>
    <input type="password" class="form-control-md" name="confirm" size ="50" required>';
            if(isset($_COOKIE["errorMatch"]))
            {
                if($_COOKIE["errorMatch"] == 1)
                {
                    echo '<br><small class="error1" class="form-text text-muted">Passwords don\'t match</small>';
                }
            }
            echo '
  </div><br><br>
