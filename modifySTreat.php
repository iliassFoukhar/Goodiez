...
//L'unicité du Username et Pseudo:
    ...
    //Insert everything into the Data Base
     $sqlInsert ="UPDATE user SET firstName='$firstName', lastName='$lastName', adress='$adress', phone='$phone', email='$email', username='$username' WHERE id_user=$idShipper";
    //Not do it until everything is set up :
    if($notUnique == 1)
        $resultat = mysqli_query($link,$sqlInsert);
    else
        header("location: modifyS.php?id=".$idShipper);    if($resultat)
