 <?php session_start();
    ...
    $idGestion = $_GET["id"];
    $sql ="SELECT * FROM user WHERE id_user = '$idGestion';";
    ...
    $email = $data["email"];
    setcookie("idGestion", $idGestion, time() + 1000);
    setcookie("exEmail", $email, time() + 1000);
    setcookie("exUsername", $username1, time() + 1000);
?>
<!DOCTYPE html>
<html lang="en">
    <head>
    ...
    <!-- NavBar -->
        ...
        <!-- add market -->
    <!-- Join Form -->
    <h2 style ="background-color:rgba(0,0,0,0.7); text-decoration : none; color:white; margin : 12px 33%;padding: 10px 10px; border-radius: 3px; border : solid 2px black;"align="center">Modify Market Admin</h2>
        <form id="joinForm" method="post" action="modifyGTreat.php" style ="color: white; background-color :rgba(0,0,0,0.6);">
  <div class="form-group" align="center">
    <label for="firstname">First Name :</label><br>
    <input type="text" class="form-control-md" name="firstName" size ="50" value=<?php echo "'".$firstName."'" ?> required>
      
  </div>
    <div class="form-group" align="center">
    <label for="lastName">Last Name :</label><br>
    <input type="text" class="form-control-md" name="lastName" value=<?php echo "'".$lastName."'" ?>  size ="50" required><br>
  </div>
    <div class="form-group" align="center">
    <label for="email">Email :</label><br>
    <input type="email" value=<?php echo "'".$email."'" ?>  class="form-control-md" name="email" size ="50" required>
  </div>
    <?php 
    ...
    <input type="text" value=<?php echo "'".$phone."'" ?>  class="form-control-md" name="phone" size ="50" required>
  </div>
        <div class="form-group" align="center">
    <label for="username">Username :</label><br>
    <input type="text" class="form-control-md" value=<?php echo "'".$username1."'" ?>  name="username" size ="50" required>
  </div>
        ...
        ?>    
            ...
            <textarea id="msg" class="form-control z-depth-1" name="adress" rows="3" cols = "20" required><?php echo $adress?> </textarea>
                </div>
                        <div id ="buttonss">
    ...
    <div class="col-6">
        <button type="submit" class="btn btn-warning">Modify</button>
            ...
            <?php echo "<a href ='removeGTreat.php?id=".$idGestion."'"; ?>
                   <button type="button" style='position: absolute; left : 50%;transform: translateX(-50%);' class="btn btn-danger">Remove</button></a></div>
</form>
